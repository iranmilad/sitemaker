                            ...
                            <div class="orders-card mt-0 h-100">
                                ...
                                <div class="table-bottom-brd table-responsive">
                                    <table class="table align-middle text-center order-table">
                                        ...
                                        <tbody>
                                            @forelse ($user->basket() as $order)
                                                <tr>
                                                    <td><span>{{ $order->cart->createdAtDate }}</span></td>
                                                    <td><span class="id">{{ $order->cart->id }}</span></td>
                                                    <td>
                                                        <span class="name">{{ $order->cart->count  }}</span>
                                                    </td>
                                                    <td><span class="price fw-500">{{ $order->cart->total }} تومان </span></td>
                                                    <td>
                                                        @if ($order->cart->orderStatus=="compeleted")
                                                            <span class="badge rounded-pill bg-success custom-badge">ارسال شد</span>
                                                        @elseif ($order->cart->orderStatus=="pending")
                                                            <span class="badge rounded-pill bg-danger custom-badge">در انتظار</span>
                                                        @elseif ($order->cart->orderStatus=="processing")
                                                    ...
                                                    <td>
                                                        <a href="/dashboard/order/{{ $order->cart->id }}" class="view"><i
                                                                class="icon anm anm-eye btn-link fs-6"></i></a></td>
                                                </tr>
                                            @empty
                                                <tr>هیچ سفارشی یافت نشد</tr>
                                            @endforelse
